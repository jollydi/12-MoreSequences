...
def run_test_draw_points_on_circle():
    ...
    title = 'DRAW_POINTS_ON_CIRCLE, test 4: 10 purple dots on aqua.'
    window = rg.RoseWindow(500, 500, title)
    circle = rg.Circle(rg.Point(250, 250), 200)
    circle.fill_color = 'aqua'
    draw_points_on_circle(window, circle, 6, 'medium purple3')
    window.close_on_mouse_click()
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(len(point_list)):
        point = point_list[k]
        circle_point = rg.Circle(point, 10)
...
def run_test_pizza():
    ...
    title = 'PIZZA test 4: 100 blue slices on grey circle.'
    window = rg.RoseWindow(500, 500, title)

    circle = rg.Circle(rg.Point(250, 250), 200)
    circle.fill_color = 'slate gray'
    pizza(window, circle, 100, 'sky blue', 2)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    circle.attach_to(window)
    for k in range(len(point_list)):
        line = rg.Line(circle.center, point_list[k])
        line.color = color
...
def run_test_polygon():
    ...
    title = 'POLYGON test 4: 12 segments with green lines'
    window = rg.RoseWindow(500, 500, title)

    circle = rg.Circle(rg.Point(250, 250), 200)
    circle.outline_thickness = 4
    circle.fill_color = 'firebrick'
    polygon(window, circle, 12, 'spring green', 6)
    window.close_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    circle.attach_to(window)
    point_list = generate_points_on_circle(circle, number_of_segments)

    for k in range(len(point_list)):
        if k == len(point_list) - 1:
            line = rg.Line(point_list[-1], point_list[0])
        else:
            line = rg.Line(point_list[k], point_list[k + 1])
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
        window.render()
...
def run_test_fancy_polygon():
    ...
    title = 'FANCY POLYGON test 4: 51 orange lines on brown circle, hops = 25'
    window = rg.RoseWindow(500, 500, title)
    circle = rg.Circle(rg.Point(250, 250), 200)
